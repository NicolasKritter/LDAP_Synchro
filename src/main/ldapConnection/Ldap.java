...
public class Ldap {
	...
	public List<Utilisateur> getUsersToSync(String DCs, String timeFilter)
			throws IOException {
		int schoolId = ConfigObject.getSchoolID();
		...
		try {
			...
			String[] attributes = { "givenName", "memberOf", "uid",
					"objectClass", "modifyTimeStamp", "userPassword", "sn" };
			...
			String filter = "(&(objectClass=" + PERSON_FILTER
					+ ")(!(modifyTimestamp<=" + timeFilter + ")))";
			NamingEnumeration values = contexte.search(DCs, filter,
					searchControls);
	...
	public void searchByType(String typeFilter, String DCs) {
		...
		try {
			...
			try {
				...
				SearchControls searchControls = new SearchControls();
				searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
				String[] attributes = { "cn", "sn", "givenName", "memberOf",
						"uid", "objectClass", "modifyTimeStamp",
						"userPassword" };
				searchControls.setReturningAttributes(attributes);
				...
				while (values.hasMoreElements()) {
					...
					if (null != attribs) {
						...
						for (NamingEnumeration ae = attribs.getAll(); ae
							...
							for (Enumeration vals = atr.getAll(); vals
									.hasMoreElements();
